...
2. **Firmware Display_HMI**  
   Mantén el mismo esquema de numeración en el código fuente del HMI (por ejemplo, texto mostrado en pantalla o constantes internas) y sincronízalo con el tag que publiques.
...
1. **Bump de versión en código**  
   ...
   - Ajusta la versión equivalente en el firmware del HMI si aplica.
...
4. **Release en GitHub**  
   ...
   - Adjunta los binarios resultantes (`firmware.bin`, binarios del HMI) y, si cambia, la tabla de particiones OTA usada (`ESP32_OTA_partition_16MB.csv`).  
   - Incluye en las notas: alcance, cambios relevantes, revisión de hardware soportada y hashes de build si los tienes.
